...
async function getRandomUser() {
    ...
    humarea = data[0].hum
    aqiarea = data[0].aqi
    currentarea = data[0].cur.map(multiplyByTwo)
    getValues(temparea, datearea, humarea, aqiarea, currentarea)
}

function getValues(temparea, datearea, humarea, aqiarea, currentarea) {
    ...
    // Area Chart Example power
    var yyyyValues = currentarea;
...
async function frequency() {
    try {
        ...
        const data = await response.json();
        areafrequency = data[0]?.[Object.keys(data[0])[0]] || 0;
    } catch (error) {
